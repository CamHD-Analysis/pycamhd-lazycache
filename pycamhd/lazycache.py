...
class LazycacheAccessor:
    ...
    def get_metadata(self, url, timeout = DEFAULT_TIMEOUT ):
        ...
        return Lazycache.get_metadata(url, timeout=timeout)

    def get_frame(self, url, frame_num, format = 'np', timeout =  DEFAULT_TIMEOUT ):
        url = self.merge_url(url)
        if self.verbose:
            print("get_frame: %s" % url)
        return Lazycache.get_frame( url, frame_num, format=format, timeout = timeout )

    def save_frame(self, url, frame_num, filename, format = 'np', timeout =  DEFAULT_TIMEOUT ):
        url = self.merge_url(url)
        if self.verbose:
            print("get_frame: %s" % url)
        return Lazycache.save_frame( url, frame_num, filename, format=format, timeout = timeout )

    def get_dir( self, url ):
        url = self.merge_url( url )
        if self.verbose:
            print("get_dir: %s" % url)
        return Lazycache.get_dir( url )
